...
namespace Utilities
{
  ...
  namespace MPI
  {
    ...
    namespace internal
    {
      ...
      /**
       * Function to simplify a many to many communication.
       *
      ...
      send_and_receive(const MPI_Comm                       &comm,
                       const std::map
                       <unsigned int, std::vector<T> >      &objects_to_send);

      /**
       * Function to simplify a collective gather operation with an arbitrary
       * number of objects.
       *
       * @param[in] comm MPI communicator.
